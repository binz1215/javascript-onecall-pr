...
import { inputWeekDayPeople } from '../view/inputView.js';

class ValidWeekdayName {
  #workDayList;

  async validWeekDayName() {
    let valid = true;

    while (valid) {
      try {
        const work = await inputWeekDayPeople();
        this.#splitWork(work);
        this.#workValidCheck();
        valid = false;
      } catch (error) {}
...
export default ValidWeekdayName;
